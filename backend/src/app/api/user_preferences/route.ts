...
// GET handler to retrieve all user preferences
export async function GET(request: NextRequest) {
    try {
        const userPreferences = await db.select().from(user_preferences);
        return NextResponse.json(userPreferences);
    } catch (error) {
        return handleErrorResponse(error as Error, 'Failed to retrieve user preferences');
    }
}
